...
/cr-vigil-monitor --declare <GitLab MR 链接>
...
| `python -m crvigil declare <MR_URL>` | 采集 MR 数据并生成预填声明模板 | JSON 摘要 |
...
收到 `/cr-vigil-monitor --declare <MR链接>` 时：

1. 执行 `python -m crvigil declare <MR链接>`。
2. 读取命令输出 JSON，向用户展示：
   - MR 基本信息（标题、作者、审查人）
   - CI 状态和 AI 声明检测结果
   - 预填好的声明 Markdown（可直接复制粘贴到 MR 描述中）
